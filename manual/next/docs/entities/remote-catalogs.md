...
- **Git hosting platforms**: GitHub, GitLab, Bitbucket Cloud, Bitbucket Server / Data Center, Gitea, Forgejo, Codeberg (using their respective APIs with token-based authentication). Each of them accepts a custom API base URL, so GitHub Enterprise Server, a self-managed GitLab or any hosted Forgejo/Gitea instance is covered by the matching source kind
...
| `bitbucket` | Sync from a Bitbucket Cloud repository |
| `bitbucketserver` | Sync from a Bitbucket Server / Data Center repository |
...
The `bitbucket` source kind is the exception: it targets the Bitbucket **Cloud** API 2.0 only. Bitbucket Server / Data Center exposes a different API (`/rest/api/1.0`) and has its own `bitbucketserver` source kind. For any forge without a supported API, use the generic `git` source kind, which clones the repository over HTTPS or SSH.
