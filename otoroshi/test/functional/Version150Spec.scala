...
class AuthModuleConfigApiSpec(name: String, configurationSpec: => Configuration)
  ...
  override def writeEntityToJson(entity: AuthModuleConfig): JsValue               = AuthModuleConfig._fmt(env).writes(entity)
...
class TeamsSpec(name: String, configurationSpec: => Configuration) extends OtoroshiSpec {
  ...
  s"[$name] Otoroshi Teams filtering" should {
    ...
    "check services number with different users" in {
      //[REMOVE SERVICEDESC] fixtures are routes now, list them through the routes api
      call("GET", "/api/routes", TenantId("test-teams"), adminUser).futureValue.as[JsArray].value.toSeq.size mustBe 4
      call("GET", "/api/routes", TenantId("test-teams"), tenantAdminUser).futureValue.as[JsArray].value.toSeq.size mustBe 3
      call("GET", "/api/routes", TenantId("test-teams"), team1User).futureValue.as[JsArray].value.toSeq.size mustBe 1
      call("GET", "/api/routes", TenantId("test-teams"), team2User).futureValue.as[JsArray].value.toSeq.size mustBe 1
      call("GET", "/api/routes", TenantId("test-teams"), team1and2User).futureValue.as[JsArray].value.toSeq.size mustBe 2
    }
    "shutdown" in {
      stopAll()
    }
