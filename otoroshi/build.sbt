...
lazy val webAuthnVersion         = "2.9.0"
...
// Maven Central enforces a monthly storage quota per namespace and a single otoroshi release used
// to weigh ~218Mb there, 157Mb of which was the scaladoc jar alone. Central still requires a
// -javadoc.jar for a deployment to validate, but explicitly allows a placeholder one, so document
// nothing: `doc` then produces an empty scaladoc and `packageDoc` a ~1Kb jar. Nothing else in the
// build consumes the scaladoc (the published manual is built by scripts/doc.sh).
Compile / doc / sources := Seq.empty

// sbt-sbom unconditionally appends the cyclonedx SBOM to packagedArtifacts, so publishSigned was
// uploading another ~850Kb to Central on every release. The SBOM is still generated by
// `Provided / makeBom` during the build and shipped as an asset of the github release, which is
// where consumers actually look for it.
packagedArtifacts := packagedArtifacts.value.filterNot { case (art, _) => art.`type` == "cyclonedx" }
